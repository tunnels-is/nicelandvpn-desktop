...
const useForm = (props) => {
    ...
    const RegisterSubmit = async () => {
        ...
        let FR = {
            Path: "v2/user/create", Method: "POST", JSONData: inputs, Timeout: 20000
        }
    ...
    const HandleSubmit = async () => {
        ...
        let FR = {
            Path: "v2/user/login", Method: "POST", Timeout: 20000, JSONData: inputs,
        }
    ...
    const ResetSubmit = async () => {
        ...
        let request = {
            Email: inputs["email"], NewPassword: inputs["password"], ResetCode: inputs["code"],
        }

        props.toggleLoading({tag: "RESET", show: true, msg: "Changing your password, please wait a moment..."})

        let FR = {
            Path: "user/reset/password", Method: "POST", JSONData: request, Timeout: 20000
        }
    ...
    const GetCode = async () => {
        ...
        let FR = {
            Path: "user/reset/code", Method: "POST", JSONData: request, Timeout: 20000
        }
...
const Login = (props) => {
    ...
    if (!props.state?.ActiveRouter) {
        return (<ScreenLoader
                loading={{logTag: "", tag: "LOGIN", show: true, msg: "Looking for a router..", includeLogs: false}}
                toggleError={props.ToggleError}></ScreenLoader>)
